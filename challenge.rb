...
def normalize(groups)
  groups.map(&:sort).sort_by { |g| g.first.to_s }
end
...
begin
  ...
  words = %w[Home homerun HOST hostel] # 'hom' and 'hos'
  res = group_by_prefix(words, 3)
  assert_eq(normalize(res), normalize([%w[Home homerun], %w[HOST hostel]]), 'Case-insensitive / keep originals')
